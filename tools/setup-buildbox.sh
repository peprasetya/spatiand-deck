...
# Two flags that are not optional any more, and both of whose failures read as packaging
# faults rather than as what they are.
#
# --overwrite '*': Arch has split gcc-libs into libasan, libgomp, libstdc++ and the rest,
# while SteamOS still ships one package owning all of those files. Downgrading, pacman sees
# two owners for a path and refuses the entire transaction. Inside a container that is thrown
# away this is exactly the right answer; on a real system it would not be.
#
# --ignore filesystem: distrobox bind-mounts /etc/hosts and /etc/host.conf, and the filesystem
# package cannot replace a mount point -- "could not rename /etc/hosts.pacnew ... Device or
# resource busy". Nothing here links against filesystem, so holding it back costs nothing.
sudo pacman -Syyuu --noconfirm --overwrite '*' --ignore filesystem >/dev/null

echo "== installing the toolchain =="
# pipewire and libmysofa are here for their headers, not their programs: spatial audio binds
# the first at build time and the second by name at run time. clang comes with them because
# the PipeWire bindings are generated by bindgen, which needs libclang -- the build fails with
# a message about libclang rather than about PipeWire, which is worth knowing in advance.
sudo pacman -S --noconfirm --needed --overwrite '*' \\
    rust base-devel pkgconf git clang \\
...
echo "== versions =="
rustc --version
pacman -Q glibc wayland libinput mesa pipewire libmysofa
INNER
